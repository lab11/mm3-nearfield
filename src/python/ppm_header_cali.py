...
class my_top_block(grc_wxgui.top_block_gui):
   def __init__(self, options):
	...
	self._bb_freq_text_box = forms.text_box(
		parent=self.nb0.GetPage(0).GetWin(),
		value=self._bb_freq,
		callback=self.setbbfreq,
		label="base band frequency",
		converter=forms.float_converter(),
	)
	self.nb0.GetPage(0).Add(self._bb_freq_text_box)
	...
	if options.playback == True:
		self.source2 = blocks.file_source(gr.sizeof_gr_complex, "iq_data_v8_chip_3_FAFA.dat", True)
	...
	#self.file_sink2 = blocks.file_sink(gr.sizeof_float, "my_iq_recording_yao_processed_mode1.dat")
	#self.connect(self.source, self.chan_filt, self.mag, self.file_sink2)
	if options.record == True:
		self.file_sink0 = blocks.file_sink(gr.sizeof_gr_complex, "calibrate.dat")
		...
		#self.connect(self.chan_filt, self.file_sink0)
   ...
   def setbbfreq(self,arg):
	self._bb_freq = float(arg)
	self.tm_framer.setbbfreq(self._bb_freq)
...
def main():
	...
	parser.add_option("-g", "--gain", type="float", default=68, help="USRP gain [default=%default]")
	...
	parser.add_option("-n", "--bb-freq", type="float", default=5, help="base band freq")
