...
%% create test vectors to stress algorithm
% test vectors can overwrite the previous statements
matload = load('test_vec_copy_5mm_data.mat');
base_vec = matload.test_vector; % demod_data = 10101

zero_vec = zeros(1,length(base_vec));

% now create test vectors that add non-idealities to the waveform

% short glitch in the prf part of data
% should be ignored - demod_data = 10101
short_pulse_data = [zeros(1,2.6e6) ones(1,4) zeros(1,length(base_vec)-2.6e6-4)];

% valid glitch in the prf part of data
% should be ignored - demod_data = 10101
reg_pulse_data = [zeros(1,2.6e6) ones(1,6) zeros(1,length(base_vec)-2.6e6-6)];

% short glitch in the prf window
% should be ignored - demod_data = 10101
short_pulse_prfwin = [zeros(1,2825000) ones(1,4) zeros(1,length(base_vec)-2825000-4)];

% long glitch in the prf window
% should be ignored - demod_data = 10101
long_pulse_prfwin = [zeros(1,2825000) ones(1,8) zeros(1,length(base_vec)-2825000-8)];

% short glitch after 1st pulse in sync window
% should be ignored - demod_data = 01010
short_pulse_sync = [zeros(1,2060000) ones(1,4) zeros(1,length(base_vec)-2060000-4)];

% valid glitch after 1st pulse in sync window
% should break the sync and result in demod_data = 01010
reg_pulse_sync = [zeros(1,2060000) ones(1,6) zeros(1,length(base_vec)-2060000-6)];

% long glitch after 1st pulse in sync window
% should be ignored - demod_data = 01010
long_pulse_sync = [zeros(1,2060000) ones(1,8) zeros(1,length(base_vec)-2060000-8)];

% short glitch after 3rd pulse in sync window
% should be ignored - demod_data = []
short_pulse_sync3 = [zeros(1,2330000) ones(1,4) zeros(1,length(base_vec)-2330000-4)];

% valid glitch after 3rd pulse in sync window
% should break demod - will never find sync header - demod_data = []
reg_pulse_sync3 = [zeros(1,2330000) ones(1,6) zeros(1,length(base_vec)-2330000-6)];

% data_pwr = or(base_vec,zero_vec);             % demod_data = 10101
% data_pwr = or(base_vec,short_pulse_data);     % demod_data = 10101
% data_pwr = or(base_vec,reg_pulse_data);       % demod_data = 10101
% data_pwr = or(base_vec,short_pulse_prfwin);   % demod_data = 10101
% data_pwr = or(base_vec,long_pulse_prfwin);    % demod_data = 10101
% data_pwr = or(base_vec,short_pulse_sync);     % demod_data = 01010
% data_pwr = or(base_vec,reg_pulse_sync);       % demod_data = 01010
% data_pwr = or(base_vec,long_pulse_sync);      % demod_data = 01010
% data_pwr = or(base_vec,short_pulse_sync3);    % demod_data = []
% data_pwr = or(base_vec,reg_pulse_sync3);      % demod_data = []

% try a few more iterations on the prf and pulse of the base vector
% matload = load('test_vec_copy_5mm_data_long_pulse.mat');
% data_pwr = matload.test_vector; % demod_data = 10001

matload = load('test_vec_copy_5mm_data_long_prf.mat');
data_pwr = matload.test_vector; % demod_data = 01010

% plot to see what we are getting
plot(data_pwr);
